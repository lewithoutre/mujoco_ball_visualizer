...
This version avoids mujoco.viewer.launch_passive() and uses raw GLFW rendering.

Important workaround for this Jetson setup:
  1. import glfw
  2. create GLFW window
  3. then import/use mujoco

Importing/using MuJoCo before creating the GLFW window may cause:
...
def load_mujoco_after_glfw_window():
    """
    Load MuJoCo only after GLFW window creation.

    On this Jetson/GLX setup, creating the GLFW window before importing/using
    MuJoCo avoids:
      GLX: No GLXFBConfigs returned
    """
    try:
...
def load_ros():
    try:
        ...
        raise SystemExit(
            ...
            "\n"
            "If your workspace path is different, source that workspace's devel/setup.bash instead."
        ) from exc
...
def draw_live_scene(
    ...
    """
    Append live debug geometry to the current MuJoCo scene.

    Important:
      Do not reset scn.ngeom here.
...
def add_bool_argument(
    ...
    """
    Python 3.8 compatible boolean optional argument.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--vision-topic",
        default=None,
        help="VisionInfo topic. Default: /dvision_<id>/VisionInfo",
    )
    parser.add_argument(
        "--raw-topic",
        default=None,
        help="Raw ball PointStamped topic. Default: /dvision_<id>/ball_raw",
    )
    parser.add_argument(
        "--filtered-topic",
        default=None,
        help="Filtered ball PointStamped topic. Default: /dvision_<id>/ball_filtered",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Hide live debug points after this many seconds without updates.",
    )
...
def create_glfw_window(width: int, height: int):
    ...
    window = glfw.create_window(width, height, "MuJoCo live ROS viewer", None, None)
...
def main() -> int:
    ...
    set_freejoint_pose(
        ...
        data,
        "ball_freejoint",
        np.array([0.0, 0.0, args.ball_radius_m]),
    )
    ...
    try:
        while not glfw.window_should_close(window) and not rospy.is_shutdown():
            ...
            with lock:
                ...
                filtered_age = (
                    now - state.last_filtered_stamp
                    if state.last_filtered_stamp
                    else float("inf")
                )
